...
#fa0 = face_alignment.FaceAlignment(face_alignment.LandmarksType.TWO_D, flip_input=False, face_detector='sfd', device='cuda')
...
def preprocess_video(input_video_path, output_video_path, face_align, mean_face_path):
    ...
    print('processing .....', len(frames))    
    landmarks = batch_detect_landmark(frames, face_align, 32)
    #landmarks = []
    #for frame in frames:
    #    landmark = detect_landmark(frame, face_align)
    #    landmarks.append(landmark)
    print('detecting .....', len(landmarks), landmarks[0].shape)
    preprocessed_landmarks = landmarks_interpolate(landmarks)
    print('interpolating .....', len(preprocessed_landmarks), preprocessed_landmarks[0].shape)    
    ...
    for i, mouth in enumerate(rois): 
        gray_mouth = cv2.cvtColor(mouth, cv2.COLOR_RGB2GRAY)
        cv2.imwrite(os.path.join('align_faces', f'fa{i+1}.jpg'), gray_mouth)  # 保存成视频帧
...
def run():
    ...
    # trainval/FI8c14giiWQ/50001.mp4
    fa = face_alignment.FaceAlignment(face_alignment.LandmarksType.TWO_D, flip_input=False, face_detector='sfd', device='cuda:0')
    #fa = face_alignment.FaceAlignment(face_alignment.LandmarksType.TWO_D, flip_input=False, face_detector='sfd', device='cpu')
    preprocess_video("test/zuYzOn0U2PY/00001.mp4", "imgs", fa, mean_face_path)
    print('Done!!!')
