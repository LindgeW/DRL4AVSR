...
class CTCLipModel(nn.Module):
    ...
    def forward(self, aud, tgt, aud_lens=None, tgt_lens=None):  # (b, t, c, h, w)
        src_lens = aud_lens

        aud = self.audio_frontend(aud)
        enc_a = self.cfm_a(aud, aud_lens)
        enc_ac = self.cfm_c(aud, aud_lens)

        aud_mean, aud_std = torch.std_mean(enc_a, dim=1)
        aud_id = self.spk_aid(torch.cat((aud_mean, aud_std), dim=-1))   # x-vector
        id_logits = self.spk_cls(aud_id)
        
        # 说话人无关
        orth_loss = diff_loss(aud_id.unsqueeze(1).expand_as(enc_ac), enc_ac)

        ctc_out = self.fc(enc_ac)
        dec_out = self.trans_dec(tgt, enc_ac, src_lens, tgt_lens)
        #out2 = self.fc(F.normalize(seq_feat1, dim=-1)) + self.fc(F.normalize(seq_feat2, dim=-1))
        return ctc_out, dec_out, id_logits, orth_loss

    def beam_search_decode(self, aud, aud_lens, bos_id, eos_id, max_dec_len=80, pad_id=0):
        with torch.no_grad():
            src_lens = aud_lens
            aud = self.audio_frontend(aud)
            enc_ac = self.cfm_c(aud, aud_lens)
            #res = beam_decode(self.trans_dec, enc_ac, src_mask, bos_id, eos_id, max_output_length=max_dec_len, beam_size=10)
            res = beam_decode(self.trans_dec, enc_ac, src_lens, bos_id, eos_id, max_output_length=max_dec_len, beam_size=10)
...
class ASRModel(nn.Module):
    def __init__(self, vocab_size, num_spk):
        ...
        self.asr = CTCLipModel(vocab_size, num_spk)
        #self.spk = SpeakerIdentity()
        self.spk = SpeakerIdentity2D(num_spk)
        self.mi_net = CLUBSample_reshape(512, 512, 512)
        self.tmp = 1.

    def forward(self, auds, tgts, spk_ids, aud_lens, tgt_lens):   # (B, T, C, H, W)
        aud_lens = (aud_lens + self.asr.scale - 1) // self.asr.scale    # time subsampling after CNN striding
        ctc_logits, dec_logits, spk_logits, drl_loss = self.asr(auds, tgts[:, :-1], aud_lens, tgt_lens)[:4]
        ...
        asr_loss = 0.9*attn_loss + 0.1*ctc_loss
        return {'asr': asr_loss, 'spk': spk_loss, 'drl': drl_loss}
    ...
    def beam_decode(self, aud, aud_lens, bos_id, eos_id, max_dec_len=50, pad_id=0):
        aud_lens = (aud_lens + self.asr.scale - 1) // self.asr.scale    # time subsampling after CNN striding
        return self.asr.beam_search_decode(aud, aud_lens, bos_id, eos_id, max_dec_len, pad_id)
