...
class CTCLipModel(nn.Module):
    ...
    def forward(self, vid, tgt, vid_lens=None, tgt_lens=None):  # (b, t, c, h, w)
        vid = self.visual_frontend(vid)
        enc_v = self.cfm_v(vid, vid_lens)
        enc_vc = self.cfm_c(vid, vid_lens)
        ...
        id_logits = self.spk_cls(vid_id)
        
        # 说话人无关
        orth_loss = diff_loss(vid_id.unsqueeze(1).expand_as(enc_vc), enc_vc)

        ctc_out = self.fc(enc_vc)
        dec_out = self.trans_dec(tgt, enc_vc, vid_lens, tgt_lens)
        #out2 = self.fc(F.normalize(seq_feat1, dim=-1)) + self.fc(F.normalize(seq_feat2, dim=-1))
        return ctc_out, dec_out, id_logits, orth_loss

    def beam_search_decode(self, vid, vid_lens, bos_id, eos_id, max_dec_len=80, pad_id=0):
        with torch.no_grad():
            vid = self.visual_frontend(vid)
            enc_vc = self.cfm_c(vid, vid_lens)
            #res = beam_decode(self.trans_dec, enc_vc, src_mask, bos_id, eos_id, max_output_length=max_dec_len, beam_size=10)
            res = beam_decode(self.trans_dec, enc_vc, vid_lens, bos_id, eos_id, max_output_length=max_dec_len, beam_size=10)
...
class VSRModel(nn.Module):
    def __init__(self, vocab_size, num_spk):
        ...
        self.vsr = CTCLipModel(vocab_size, num_spk)
        #self.spk = SpeakerIdentity()
        self.spk = SpeakerIdentity2D(num_spk)
        self.mi_net = CLUBSample_reshape(512, 512, 512)
        self.tmp = 1.

    def forward(self, vids, tgts, spk_ids, vid_lens, tgt_lens):   # (B, T, C, H, W)
        ctc_logits, dec_logits, spk_logits, drl_loss = self.vsr(vids, tgts[:, :-1], vid_lens, tgt_lens)[:4]
        spk_loss = F.cross_entropy(spk_logits, spk_ids, ignore_index=-1)
        ctc_log_probs = ctc_logits.transpose(0, 1).log_softmax(dim=-1)  # (T, B, V)
        ctc_loss = F.ctc_loss(ctc_log_probs, tgts[:, 1:], vid_lens.reshape(-1), tgt_lens.reshape(-1), zero_infinity=True)
    ...
    def beam_decode(self, vid, vid_lens, bos_id, eos_id, max_dec_len=50, pad_id=0):
        return self.vsr.beam_search_decode(vid, vid_lens, bos_id, eos_id, max_dec_len, pad_id)
